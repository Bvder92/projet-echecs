...
int *get_legal_roi(int position, int *moves)
{
    ...
    if ((colonne == 0))
        moves[0] = moves[3] = moves[5] = -1; // colonne 0 -> on peut pas aller en -9, -1 ou 7
    if ((colonne == 7))
        moves[2] = moves[4] = moves[7] = -1; // colonne 7 -> on peut pas aller en -7, 1 ou 9
    if ((ligne == 0))
        moves[0] = moves[1] = moves[2] = -1; // ligne 0 -> on peut pas aller en -9, -8, -7
    if ((ligne == 7))
        moves[5] = moves[6] = moves[7] = -1; // ligne 7 -> on peut pas aller en 7 8 ou 9
